...
class Evaluate(object):
    ...
    def load_model(self):
        if ".ckpt" not in self.ckpt_path:
            ...
            if not valid_ckpts:
                raise FileNotFoundError(
                    f"No checkpoint with 'Recall=' in name found under "
                    f"{os.path.join(self.ckpt_path, 'checkpoints')}. "
                    "Pass a direct --ckpt_path to a .ckpt file."
                )
        ...
        assert (hparams['dataset_type'] == self.dataset_type) and (hparams['sat_type'] == self.sat_type), \
            f"Checkpoint mismatch: expected dataset_type={self.dataset_type!r}, sat_type={self.sat_type!r}; " \
            f"got dataset_type={hparams['dataset_type']!r}, sat_type={hparams['sat_type']!r}"
    ...
    def get_final_metrics(self):
        ...
        retrieval_results_I2S = get_retrieval_metrics(modality1_emb=l2normalize(sat_query_embeddings), modality2_emb=l2normalize(audio_gallery_embeddings), normalized=True,k=R_k)
        retrieval_results_S2I = get_retrieval_metrics(modality1_emb=l2normalize(audio_query_embeddings), modality2_emb=l2normalize(sat_gallery_embeddings), normalized=True,k=R_k)
...
def parse_results(results_dict,R_k,args,ckpt_path):
        ...
        for key in results_dict.keys():
            ...
            update_dict['I2S_Recall'] =  round(result['retrieval_results_I2S'][recall_key(R_k)],3)
            update_dict['I2S_Median_Rank'] = int(result['retrieval_results_I2S']['Median Rank'])
            update_dict['S2I_Recall'] =  round(result['retrieval_results_S2I'][recall_key(R_k)],3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_path', type=str, default='',
                        help='Direct path to a .ckpt file or checkpoint directory. '
                             'Takes precedence over --expr / ckpt_cfg lookup.')
