...
from src.dataloader import Dataset_soundscape, collate_batch, load_llava_caption_df
...
class Evaluate(object):
    def __init__(self,split, ckpt_path,device,caption_type="audio",test_zoom_level=1, test_mel_index=0, recall_at=10,dataset_type="GeoSound", sat_type="bingmap", metadata_type=None):
        super().__init__()
        set_seed(56)
        self.split = split
        ...
        self.metadata_type = metadata_type  # None = use checkpoint value
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_path', type=str, default='',
                        help='Direct path to a .ckpt file. Takes precedence over --expr.')
    parser.add_argument('--metadata_type', type=str, default=None,
                        help='Override the metadata_type stored in the checkpoint '
                             '(e.g. for ablation). Default: use checkpoint value.')
    ...
    evaluation = Evaluate(split=args.split, ckpt_path=ckpt_path,device=device, 
                          ...
                          sat_type=args.sat_type, metadata_type=args.metadata_type)
